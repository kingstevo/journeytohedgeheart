...
function create() {
    ...
    if (gameState === 'before') {
        ...
        playButton.on('pointerdown', () => {
            // when clicked change the gamestate
            gameState = 'during';
            platformSpeed = 2;
            // kill button
            playButton.destroy();  // Destroy the graphic
            playButton = null;     // Set to null to avoid errors on multiple clicks
            console.log('Graphic removed via click!');
        });
    }
    ...
    this.input.keyboard.on('keydown-SPACE', function (event) {
        if (gameState === 'before') {
            ...
            playButton.destroy();  // Destroy the graphic
            playButton = null;     // Set to null to avoid errors on multiple clicks
            console.log('Graphic removed via space!');
...
function addObstacle() {
    if (gameState === 'during') {
        ...
        let obstacles = [
            ...
            ['crab', 0.5, 450, gravity, 80, false, 9, 3600],
        ...
        obstacle.body.setVelocityX(-platformSpeed * platToVelFactor * obstacles[obCh][1]);
...
function hitObstacle(player, obstacle) {
    ...
    playAgainButton.on('pointerdown', () => {
        ...
        // Reset the game over flag
        gameState = 'during';
        // Optionally reset the score or any other game variables
        score = Math.round((dateOfMeetingInSeconds - Date.now()) / 1000);
        obstaclesArray = [];
    });
